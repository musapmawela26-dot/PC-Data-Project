...
select continent,
       count(sale_price) as sales_record
from [PC_Salesdb].[dbo].[PC_Data]
group by Continent
...
select Payment_Method,
       count(Sale_Price) as sale_price_by_payment_method
from [PC_Salesdb].[dbo].[PC_Data] 
...
select  Channel,
       count(Sale_Price) as sale_price_online_vs_offline
from [PC_Salesdb].[dbo].[PC_Data] 
group by Channel
...
select Priority,
       count(Sale_Price) as sale_price_priority
from [PC_Salesdb].[dbo].[PC_Data] 
group by Priority
...
select sum((Sale_Price - Cost_Price) * 100/Sale_Price) as total_profit_margin
from [PC_Salesdb].[dbo].[PC_Data]
...
select top 1
       Continent,
       sum(sale_price) as total_revenue_by_continent
from [PC_Salesdb].[dbo].[PC_Data]
group by Continent
...
select ram,
       AVG(sale_price) as avg_sale_price_by_ram
from [PC_Salesdb].[dbo].[PC_Data]
group by ram
...
select top 1
       PC_Model,
       SUM(sale_price) as highest_sale_price_by_pc_model
from [PC_Salesdb].[dbo].[PC_Data]
group by PC_Model

-- 26. Calculate the average number of days between Purchase Date and Ship Date.

select avg(datediff(ship_date,purchase_date)) as avg_days
from [PC_Salesdb].[dbo].[PC_Data]
...
select top 1
       ...
       SUM(sale_price) as highest_revenue_by_dept
from [PC_Salesdb].[dbo].[PC_Data]
group by Sales_Person_Department
order by highest_revenue_by_dept desc

-- 28. Calculate total revenue per Storage Capacity.

select Storage_Capacity,
       SUM(sale_price) as total_revenue_by_storage
from [PC_Salesdb].[dbo].[PC_Data]
...
SELECT sales_person_name,total_sales_per_employee,
       RANK() OVER(PARTITION BY total_sales_per_employee order by sales_person_name) AS employee_rank
FROM [PC_Salesdb].[dbo].[PC_Data]
